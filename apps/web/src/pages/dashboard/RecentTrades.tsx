...
export function RecentTrades() {
  ...
  useEffect(() => {
    tradeService.getRecent().then(setTrades).finally(() => setIsLoading(false));
  }, []);
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Recent Trades</h1>
        <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
          Your most recent executed trades
        </p>
      </div>
      ...
      {trades.length > 0 ? (
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                {trades.map((t) => (
                  <tr key={t.id}>
                    ...
                    <td className="px-6 py-3 text-right text-gray-600 dark:text-gray-400">{t.entryPrice}</td>
        ...
        <Card>
          <EmptyState
            ...
            description="Your trades will appear here once you start executing."
          />
        </Card>
      )}
    </div>
